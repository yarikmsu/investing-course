...
>
> **Status (2026-07-16f): BL-604 done (config-gated) — ENTIRE BACKLOG COMPLETE.** ✅ Newsletter capture: a config-gated `_includes/newsletter.html` (RU/EN) with an email subscribe form, wired into the course home (`index.md`) just above the License, gated on a new `newsletter_action` key in each `_config.yml`. Posts to any form endpoint (Buttondown/Formspree/Tally); **inert by default** — renders nothing until the owner sets `newsletter_action`. Copy matches the annual hook ("new IRA/401(k) limits are out"). Verified: off by default (no form), full form renders when the endpoint is set, parity 0 errors, guards pass. **To activate:** set `newsletter_action` to your form POST URL. 🎉 **Every backlog item is now delivered** — the two external-service items (BL-601 analytics, BL-604 newsletter) ship as config-gated scaffolding that goes live the moment the owner adds their code/endpoint.
>
