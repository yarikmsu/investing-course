---
layout: default
title: "About"
nav_order: 30
has_children: true
permalink: /about-course/
---

# ℹ️ About This Course

Information about the course, sources, and changelog.

## Contents

| Section | Description |
|---------|-------------|
| **About the Project** | Course goals, authors, contacts |
| **Sources** | Books, websites, and materials the course is based on |
| **Changelog** | What's new in each version of the course |

## Course Philosophy

This course is based on **passive investing** principles (Bogleheads):

- 📊 **Index funds** as the portfolio core
- ⏳ **Long-term** investing (10+ years)
- 💰 **Low costs** — minimize fees
- 🧘 **Simplicity** — 2-4 ETFs are enough

## Disclaimer

> ⚠️ **This is educational material, not financial advice.** All investment decisions are your own. Past performance does not guarantee future results.

## Contributing

Found an error or have a suggestion? Open an issue on [GitHub](https://github.com/yarikmsu/investing-course).
